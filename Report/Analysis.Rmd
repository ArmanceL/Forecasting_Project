...
Dynamic regression --> pas d'erreur mais ne sort pas de forecast 
...
future_Gland_fin_tsibble <- new_data(Gland_fin_tsibble, 8) %>% mutate(temp = mean(Gland_fin_tsibble$temp)) 
...
```{r}
#library(forecast)
fit <- auto.arima(Gland_fin_tsibble[,"Bike_Eikenott"], xreg=Gland_fin_tsibble[,"temp"])

#checkresiduals(fit) #quel package ? 

fcast <- forecast(fit, xreg = rep(0.8,8))
autoplot(fcast) + xlag("Date") + ylag("Bike_Eikenott")
```
