...
report(fitLT)
fitLT %>% forecast(h = 234) %>% autoplot(Gland_fin_2_tsibble)

fit.LT <- Gland_fin_2_tsibble %>%  model(ARIMA(Ebike_Les_Tuillières))
report(fit.LT)
fit.LT %>% forecast(h = 234) %>% autoplot(Gland_fin_2_tsibble)

Tab1 <- predict(fit.LT, h = 234, ci = 0.90)
Tab1 <- Tab1[-c(1,3:4)]
```
...
var.pred <- predict(var.model_lev, n.ahead = 234, ci = 0.90)
