...
model(ETS(Bike ~ error("A") + trend("N", alpha = 0.5) + season("A")))
fit %>% forecast(h = 36) %>% autoplot(Gland_Les_Tuillières_Bike_tsibble, level = 0) +
...
model(ETS(E_Bike ~ error("A") + trend("A", alpha = 0.5) + season("A")))
fit %>% forecast(h = 36) %>% autoplot(Gland_Les_Tuillières_E_Bike_tsibble, level = 0) +
theme(legend.position = "none")
```
# 2. Gland Swissquote
```{r}
