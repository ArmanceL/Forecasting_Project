...
```{r}
fitLT <- Gland_fin_2 %>%  model(ARIMA(Bike_Les_Tuillières))
report(fitLT)
fitLT %>% forecast(h = 144) %>% autoplot(Gland_fin_2)

fit.LT <- Gland_fin_2 %>%  model(ARIMA(Ebike_Les_Tuillières))
report(fit.LT)
fit.LT %>% forecast(h = 144) %>% autoplot(Gland_fin_2)
